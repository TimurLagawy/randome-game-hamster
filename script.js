...
console.log(
  "Игра 'слови хомяка': нужно кликать мышкой по хомяку над мешками с зерном. Увеличенная площадь 'попадания' - можно нажать на мешок, тоже засчитывается. Продолжается до достижения разницы 20 между числом появления хомяков и успешных попаданий. После проигрыша, появляется окно 'game over', где можно ввести своё имя, хомяк в это время смеётся. Есть мелодия игры, которую можно включить/выключить, выбор уровня - от 6 до 12 мешков/ появляющихся хомяков, таблица результатов прошлых игр с именами (если было введено) игроков, есть кнопка паузы и выбор другой игры 'крестики-нолики' . Для мобильника также адаптирована."
);
